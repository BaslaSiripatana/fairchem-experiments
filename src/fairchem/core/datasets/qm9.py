    ...
    def download(cls, args: argparse.Namespace):
        ...
        def _dump_idx(indices: np.ndarray):
            ...
            for idx in indices:
                ...
                # QM9 targets: already shape (1, 19)
                y = system["y"].clone().float()   # (1, 19)

                natoms = atomic_numbers.shape[0]

                data_object = AtomicData(
                    atomic_numbers=atomic_numbers,          # (natoms,)
                    pos=pos,                                # (natoms, 3)
                    natoms=torch.tensor(natoms, dtype=torch.long),

                    # ---- required graph / PBC fields ----
                    cell=torch.eye(3, dtype=torch.float).unsqueeze(0) * 1000.0,  # (1,3,3)
                    pbc=torch.zeros((1, 3), dtype=torch.bool),                   # (1,3)
                    cell_offsets=torch.zeros((0, 3), dtype=torch.float),
                    edge_index=torch.zeros((2, 0), dtype=torch.long),
                    nedges=torch.tensor([0], dtype=torch.long),

                    # ---- required metadata ----
                    charge=torch.zeros(natoms, dtype=torch.float),
                    spin=torch.zeros(natoms, dtype=torch.float),
...
if __name__ == "__main__":
    ...
    download_parser.set_defaults(func=QM9.download)
